...
export async function refreshMoveCaseTasks(caseId: string) {
  ...
  try {
    ...
    const contextResult = await client.query<SyncContextRow>(
      `
      SELECT
        mc.id AS case_id,
        mc.user_id,
    ...
    const relevantTemplatesResult = await client.query<RelevantTemplateRow>(
      `
      SELECT
        t.id AS template_id,
        t.category,
    ...
    const existingTasksResult = await client.query<ExistingTaskRow>(
      `
      SELECT
        id,
        template_id,
    ...
    const doneTemplateIds = new Set(
      existingTasks
        .filter((task: { status: string; template_id: any; }) => task.status === 'done' && task.template_id)
        .map((task: { template_id: any; }) => String(task.template_id))
    );

    const openTaskByTemplateId = new Map<string, ExistingTaskRow>();
    ...
    const relevantTemplateIds = new Set(
      relevantTemplates.map((template: { template_id: any; }) => String(template.template_id))
    );
