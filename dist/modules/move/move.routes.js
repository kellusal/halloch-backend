...
const move_task_sync_1 = require("./move.task-sync");
const router = (0, express_1.Router)();
function buildMoveCaseResponse(row) {
    const missingFields = [];
    ...
    const profileCompletenessPercent = totalChecks === 0 ? 100 : Math.round((completedChecks / totalChecks) * 100);
    return {
        ...row,
...
function normalizeSwissDateToIso(value) {
...
router.post('/cases', requireAuth_1.requireAuth, async (req, res) => {
    const client = await pool_1.pool.connect();
    let transactionOpen = false;
    try {
        const { fromCity, toCity, moveDate, hasCar, hasChildren, hasDog, fromStreet, fromHouseNumber, fromZip, toStreet, toHouseNumber, toZip, childrenCount, maritalStatus, healthInsuranceName, employerName, } = req.body ?? {};
      ...
      `, [String(toCity).trim()]);
        ...
        const insertCaseResult = await client.query(`
      ...
      `, [
        ...
        await (0, move_task_sync_1.refreshMoveCaseTasks)(createdCase.id);
        const refreshedCaseResult = await pool_1.pool.query(`
      SELECT
        mc.id,
        mc.user_id,
        mc.from_city_id,
      ...
      `, [createdCase.id]);
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error creating move case:');
    }
    finally {
        client.release();
    }
});
router.get('/cases/current-active', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const userId = req.user?.id;
        if (!userId) {
            return res.status(401).json({
      ...
      `, [userId]);
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error loading current active move case:');
    }
});
router.get('/cases/:caseId', requireAuth_1.requireAuth, async (req, res) => {
    try {
      ...
      `, [caseId, userId]);
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error loading move case:');
    }
});
router.patch('/cases/:caseId', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { caseId } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const caseCheck = await pool_1.pool.query(`
      SELECT
        mc.id,
        mc.user_id,
        mc.from_city_id,
      ...
      `, [caseId, userId]);
        ...
        const { fromStreet, fromHouseNumber, fromZip, toStreet, toHouseNumber, toZip, hasCar, hasChildren, hasDog, childrenCount, maritalStatus, healthInsuranceName, employerName, } = req.body ?? {};
      ...
      `, [
        ...
        await (0, move_task_sync_1.refreshMoveCaseTasks)(caseId);
        const updatedCaseResult = await pool_1.pool.query(`
      SELECT
        mc.id,
        mc.user_id,
        mc.from_city_id,
      ...
      `, [caseId]);
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error updating move case:');
    }
});
router.get('/cases/:caseId/tasks', requireAuth_1.requireAuth, async (req, res) => {
    try {
      ...
      `, [caseId, userId]);
        ...
        const tasksResult = await pool_1.pool.query(`
      SELECT
        id,
        case_id,
        template_id,
      ...
      `, [caseId]);
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error loading move tasks:');
    }
});
router.get('/cases/:caseId/tasks/:taskId', requireAuth_1.requireAuth, async (req, res) => {
    try {
        ...
        const taskResult = await pool_1.pool.query(`
        SELECT
          id,
          case_id,
          template_id,
        ...
        const nextTaskResult = await pool_1.pool.query(`
        SELECT id
        FROM move_case_tasks
        WHERE case_id = $1
          AND status = 'open'
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error loading move task:');
    }
});
router.patch('/cases/:caseId/tasks/:taskId/complete', requireAuth_1.requireAuth, async (req, res) => {
    try {
        const { caseId, taskId } = req.params;
        const userId = req.user?.id;
        if (!userId) {
        ...
        const updateResult = await pool_1.pool.query(`
        ...
        return (0, error_middleware_1.sendInternalServerError)(res, error, 'Error completing move task:');
    }
});
