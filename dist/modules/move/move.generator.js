...
exports.generateTasksForCase = generateTasksForCase;
...
function getDefaultLocalizedTitle(template) {
    return (template.title_de ||
        template.title_fr ||
        template.title_en ||
        'Aufgabe');
}
function getDefaultLocalizedDescription(template) {
    return (template.description_de ||
        template.description_fr ||
        template.description_en ||
        null);
}
function getDefaultLinkLabel(link) {
    return link?.label_de || link?.label_fr || link?.label_en || null;
}
async function generateTasksForCase(caseId) {
    ...
    try {
      ...
      `);
        ...
        `, [templateIds]);
            linksByTemplateId = new Map(linkResult.rows.map((row) => [row.task_template_id, row]));
        }
        ...
        const cityServiceResult = await client.query(`
        SELECT
          id,
          city_id,
          category,
